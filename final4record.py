...
def _watering(seconds):
    ...
    while watered_sec<seconds:
        ...
        #     checking=True
        #     switch.turn_off()
        #     reopen_timer=Timer(after_seconds,switch.turn_on)
...
def update_eto(*argc, **argv):
    ...
    hour_id+=1
    update_local_th()
    CIMIS_data=CIMIS.get_data()
    prev_hour_id=hour_id-1
    local_tmp,local_hum=avg_data[prev_hour_id]
    rate1,rate2=CIMIS_data[CIMIS.AirTmp]/local_tmp,CIMIS_data[CIMIS.RelHum]/local_hum
    CIMIS_data[CIMIS.Eto]=0.2#temp
    local_eto=CIMIS_data[CIMIS.Eto]*(rate1+rate2)/2
    ...
    already_water+=water_for_hour
    start_watering(get_water_seconds(water_for_hour))
    if left_hour<=0:
        left_hour=24
        already_water=0
